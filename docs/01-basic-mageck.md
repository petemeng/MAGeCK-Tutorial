...
```bash
mkdir -p full/external/brunello
curl -L 'https://media.addgene.org/cms/filer_public/f8/f7/f8f73d69-94f5-4382-bf5f-658c78e38f2f/broadgpp-brunello-library-contents.txt' -o full/external/brunello/library_raw.txt
```
...
raw_path = Path('full/external/brunello/library_raw.txt')
lib_path = Path('full/external/brunello/library.tsv')
meta_path = Path('full/external/brunello/library_metadata.tsv')
counts = Counter()
...
```bash
base='full/raw/article1_basic_full_raw'
