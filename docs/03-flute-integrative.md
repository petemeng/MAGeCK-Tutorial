...
# CRISPR 筛选最佳实践系列（三）：MAGeCKFlute 整合分析——基因筛选的全景图
...
> ✅ 2026-03-15 实跑修订
> - 可执行版代码：`repro/analysis/03_mageckflute_analysis.R`
> - 真实结果目录：`repro/results/`
> - 当前环境下 `FluteRRA()` 原始参数签名和 KEGG 在线调用都需要调整；已在 `repro` 中修复

---

## 本篇目标

前两篇我们拿到了基因列表——1,687 个 essential genes、234 个 enriched genes。但列表本身只是起点。审稿人会追问三个层面的问题：
...
![图 1：RankView](assets/figures/pub_flute_rankview.png)
...
![图 2：富集图](assets/figures/pub_flute_kegg.png)
...
![图 3：CN bias](assets/figures/pub_cn_bias.png)

**图 3：拷贝数 vs sgRNA dropout 的关系。** 横轴为 log2 拷贝数，纵轴为阴性筛选 LFC。红色回归线显示了轻微的负相关（r ≈ -0.12）——高拷贝数基因确实倾向于更大的 dropout。但相关性较弱，说明 HeLa 的 CN bias 在可控范围内。
...
>
> 如果 |r| < 0.15（像我们的 HeLa 数据），不校正也可以——但在论文中应该报告你检查过 CN bias 并说明为什么不需要校正。
...
![图 4：SquareView 风格图](assets/figures/pub_flute_squareview.png)
...
![图 5：DepMap-demo 热图](assets/figures/pub_depmap_heatmap.png)
...
| 指标 | 我们的分析 | 原文 (Wang 2022) | DepMap HeLa |
|------|-----------|------------------|-------------|
| Essential genes (FDR<0.05) | 1,687 | ~1,750 | ~1,800 |
| Top 1 essential | RPS19 | RPL11 | RPS19 |
| Top enriched | CDKN1A | TP53 | CDKN1A |
| KEGG Top 通路 | Ribosome | Ribosome | Ribosome |
| CN bias (r) | -0.12 | 未报告 | N/A |

三个独立来源的结果高度一致——这给了我们对分析流程的充分信心。
...
**拷贝数校正** 检查了 HeLa 的 CN bias（r = -0.12，在可控范围内）。在基因组更不稳定的细胞系中，这一步是必须的。

**DepMap 整合** 显示我们的 Top 20 essential genes 在 83-99% 的 DepMap 细胞系中也被判定为 essential——它们是真正的 pan-essential genes。
