...
这里也先把数据性质说清楚：第 4 篇不是前 1–3 篇那种 single-accession 的全量公开 raw reanalysis，而是仓库内维护的**教学级 count table**。它保留了典型 CRISPRi / CRISPRa screen 的结构和信号方向，但规模只有约 **1000 个基因 × 每基因 5 条 sgRNA = 5000 条 sgRNA**，目的是稳定演示分析逻辑和图表解读，而不是复刻一个全基因组文库的 hit 数。
...
- CRISPRi 数据里所有 **5000** 条 sgRNA 都带有 TSS 距离信息，对应约 **1000** 个基因，是一个教学规模而不是全基因组文库
...
这次脚本里利用本教学数据集的命名约定（例如 `A1CF_TSS1_+14_g3`）直接从 sgRNA 名称中解析了 TSS 偏移，并基于 `results/crispri_test.normalized.txt` 的归一化计数计算展示用 LFC：

- `lfc = log2((T12_rep1 + T12_rep2 + 1) / (T0_rep1 + T0_rep2 + 1))`

这不是所有真实文库都会自带的信息。很多 CRISPRi 项目里的 sgRNA 名称并不编码 TSS 距离，那时通常需要再用 guide 坐标和 TSS 注释文件自己做 mapping。

### 图 1：TSS 距离与 CRISPRi 效率

![图 1：TSS 距离](assets/figures/pub_tss_distance.png)

这张图是本篇最重要的一张。阴影区域标出的是常用经验窗口（约 `-50` 到 `+300 bp`）；这个范围本质上沿用了 `Gilbert et al., 2014` 和 `Horlbeck et al., 2016` 里最常被引用的 CRISPRi 设计经验。真实结果能看到：
...
但对这套**小规模教学数据**来说，更准确的说法是：这一步的统计功效其实很有限，`0.044 vs 0.002` 更适合作为“流程演示”而不是实质性证据。也就是说：

- ko-only hits 的 CN 均值方向上确实偏高
- 但这个教学规模还不足以把 CN 偏差讲成一个很强的统计结论

所以这一步的主要价值，是教你在真实项目里看到“ko-only + 放大区域”时要主动起疑；真正到了全基因组 screen，这类 CN 偏差检查才会更有说服力。
...
- `results/crispra_test.normalized.txt`

并基于 MAGeCK 写出的归一化计数计算展示用 LFC：
...
像 `BHLHE40`、`CASP2`、`AIFM1` 这类偏应激 / 生长调控方向的基因落在右尾，至少不会让人觉得方向完全违和；但这套教学数据的重点仍然是让你看懂 **CRISPRa 的右尾很稀疏**，而不是把这份 top list 外推成某个特定药物机制结论。
