...
```bash
python full/scripts/sanitize_mle_count_table.py \
  -i full/counts/article1_basic_full_raw/mageck_count.count.txt \
  -o full/counts/article2_full_raw/mageck_count.mle_compatible.txt
```
...
这里把 `--permutation-round` 设成 `2`，主要是为了把全量教程的实跑时间控制在可接受范围内。正式项目里如果你更关心 p 值 / FDR 的稳定性，通常建议把它提高到 `10` 或更多；本篇先用 `2`，重点是把 design matrix、beta 和结果解读讲清楚。
...
这里故意同时加了 **效应大小** 和 **显著性** 两道门槛。`|beta| > 0.5` 不是一条神奇金线，而是 CRISPR screen 里很常见的经验 cut-off：先把大量接近 0 的弱波动筛掉，把注意力集中到方向更明确的基因上。正式项目里，你也可以结合 beta 分布拐点、已知阳性对照和实验容忍的假阳性率再调整这个阈值。
...
这里最容易让人皱眉的是：第 1 篇 RRA 只有 `5` 个 positive hits，而 MLE 这里给出了 `80` 个 enriched hits。两者不能直接按“16 倍”去理解，因为判定规则本来就不同：RRA 用的是 `pos|fdr < 0.05`，MLE 用的是 `beta > 0.5` 且 `fdr < 0.05`。参数模型的 MLE 对弱而一致的富集信号更敏感，所以更容易把 positive 端候选放大；也正因为如此，这 80 个 enriched genes 更适合作为候选池，再回到 sgRNA 一致性和细胞背景去复核，而不是直接照单全收。
...
像 `LATS2`、`NF2` 这些 top positive hit 的 `fdr` 显示成 `0.0000000`，不是因为 p 值真的等于 0，而是结果表打印精度把一个非常小的正数截断成了 0。把它理解成“极显著”是对的，但不要把它字面当成零概率事件。
...
正向 hits 也不是完全没有生物学逻辑。`LATS2`、`NF2` 更偏 Hippo / 生长抑制方向，`CDKN2A`、`DLC1` 也带有“细胞刹车失活后可能获得增殖优势”的味道；但由于 MLE 在 positive 端明显比 RRA 更敏感，这些基因更适合先作为高优先级候选，再结合 sgRNA 层面和 A375 背景去判断，而不是直接下结论。

---

## Step 5：MLE 和 RRA 到底有多一致

我把第 1 篇的 RRA 结果和这次 MLE 结果做了逐基因合并统计。这里的口径故意和 Step 4 保持一致：MLE 仍然用 `beta < -0.5` / `beta > 0.5` 配合 `fdr < 0.05` 来定义命中。
...
另外，`common genes = 19114` 这个数字不是算错，而是两边对 non-targeting controls 的记法不同：RRA 结果里它们是 1000 个单独的 `Control_sg*` 条目，MLE 结果里则保留成 1 个 `Non-Targeting_Control` 汇总行。因此真正能按“基因名”直接一一对齐的共有条目就是 **19,114**。

### 图 1：MLE vs RRA 散点

这张图把每个基因的 `RRA negative-selection LFC` 和 `MLE beta` 放在同一个坐标系里。为了避免极端离群 beta 把整张图压扁，展示时把 y 轴裁到了 ±6；这次真正被裁掉的其实只有 **1** 个基因，也就是前面 FAQ 会提到的 `MTPAP`。
...
这一节我只保留一个很实用的结论：确认 `mageck-vispr init` 能在真实 FASTQ 上生成可编辑的 workflow scaffold；完整的 `config.yaml` 补齐、`snakemake` 执行和交互界面展开不在本篇继续拉长。
...
```
📊 输出：
60K   full/reports/figures/article2_pub_beta_distribution_full.png
...
这很常见，但别直接把它理解成“MLE 比 RRA 多检出 16 倍真阳性”。RRA 的判定是 `pos|fdr < 0.05`，MLE 这里的判定是 `beta > 0.5` 且 `fdr < 0.05`；统计框架不同，MLE 对弱而一致的富集信号更敏感，所以 positive 候选会明显变多。对这 80 个 enriched genes，更稳妥的做法是回到 sgRNA 一致性、细胞背景和后续验证，而不是只盯着 gene-level `fdr`。
...
| 第 4 篇 | CRISPRi/CRISPRa 筛选分析策略 | 已切换为真实教学版 |
| 第 5 篇 | 药物-基因互作筛选与合成致死分析 | 已切换为真实教学版 |
| 第 6 篇 | 发表级图表与审稿人常见问题 | 已更新为收官版 |
