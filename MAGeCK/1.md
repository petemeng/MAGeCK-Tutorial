...
仓库里已经把全量工作流整理在 `MAGeCK/full/`。从仓库根目录执行：

```bash
# 1) 下载并转换 Brunello 文库注释
python MAGeCK/full/scripts/fetch_brunello_library.py

# 2) 下载 article 1 需要的全部 FASTQ，并自动把 lane-split run 合并成样本级文件
bash MAGeCK/full/scripts/download_sra.sh --cohort article1_basic_full_raw
...
这一批数据我先在 pDNA 样本上做了 `--test-run` 探测，最后验证得到：

- **方向**：正向匹配即可，不需要 reverse-complement
- **最佳 `--trim-5`**：`23,24,25,26,27,28,29,30`

也就是说，这一批 reads 里 sgRNA 起始位并不完全固定在单一偏移量上，而是落在一个小窗口里。这个设置对比对率影响非常大。
...
- 这其实很符合 dropout screen 的经验：**必需基因更容易在终点样本中“掉下去”，而真正能给细胞明显生长优势的敲除位点往往少得多**
...
```bash
Rscript MAGeCK/full/scripts/plot_article1_full.R
```
...
很多公开 screen 会同时提供 pDNA 和细胞内基线样本。pDNA 更接近“文库初始分布”，常用于最基础的入口分析；但如果实验设计里有明确的早期细胞基线，是否改用 T0/T1 作为 control，需要结合具体实验方案判断。
...
| 第 2 篇 | MAGeCK MLE + VISPR——复杂实验设计与交互可视化 | 已整理教程稿 |
| 第 3 篇 | MAGeCKFlute 整合分析——基因筛选的全景图 | 已整理教程稿 |
| 第 4 篇 | CRISPRi/CRISPRa 筛选分析策略 | 已整理教程稿 |
| 第 5 篇 | 药物-基因互作筛选与合成致死分析 | 已整理教程稿 |
| 第 6 篇 | 发表级图表与审稿人常见问题 | 已整理教程稿 |
